...
CDemoModel::CDemoModel()
{
    ...
    m_ColorDifusoC = NULL;
    m_ColorDifuso = XMFLOAT3(0.0f, 0.0f, 1.0f);

    m_pMagnitudEspecular = NULL;
    magnitudEspecular = 32;

    m_pMagnitudEspecular2 = NULL;
    magnitudEspecular2 = 32;
}
...
bool CDemoModel::LoadContent()
{
    ...
    hr = m_pD3DDevice->CreateBuffer(&constBufferDesc, 0, &m_ColorDifusoC);
    if (FAILED(hr)) {
        return false;
    }

    //buffer para la magnitud especular
    hr = m_pD3DDevice->CreateBuffer(&constBufferDesc, 0, &m_pMagnitudEspecular);
    if (FAILED(hr)) {
        return false;
    }

    hr = m_pD3DDevice->CreateBuffer(&constBufferDesc, 0, &m_pMagnitudEspecular2);
    if (FAILED(hr)) {
        return false;
    }
...
void CDemoModel::UnloadContent()
{
    ...
    m_pCameraPosCB = NULL;
    if (m_ColorDifusoC)
        m_ColorDifusoC->Release();
    m_ColorDifusoC = NULL;
    if (m_pMagnitudEspecular)
        m_pMagnitudEspecular->Release();
    m_pMagnitudEspecular = NULL;
    if (m_pMagnitudEspecular2)
        m_pMagnitudEspecular2->Release();
    m_pMagnitudEspecular2 = NULL;
}
...
void CDemoModel::Update()
{
    ...
    if (m_ColorDifuso.x > 0.0f && m_ColorDifuso.y < 1.0f && m_ColorDifuso.z < 0.1f) {
        m_ColorDifuso.x -= 0.0001f;
        m_ColorDifuso.y += 0.0001f;
    }

    if (m_ColorDifuso.x < 0.1f && m_ColorDifuso.y > 0.0f && m_ColorDifuso.z < 1.0f) {
        m_ColorDifuso.y -= 0.0001f;
        m_ColorDifuso.z += 0.0001f;
    }

    if (m_ColorDifuso.x < 1.0f && m_ColorDifuso.y < 0.1f && m_ColorDifuso.z > 0.0f) {
        m_ColorDifuso.z -= 0.0001f;
        m_ColorDifuso.x += 0.0001f;
    }
}
...
void CDemoModel::Render()
{
    ...
    m_pD3DContext->UpdateSubresource(m_ColorDifusoC, 0, 0, &m_ColorDifuso, 0, 0);
    m_pD3DContext->UpdateSubresource(m_pMagnitudEspecular, 0, 0, &magnitudEspecular, 0, 0);
    m_pD3DContext->UpdateSubresource(m_pMagnitudEspecular2, 0, 0, &magnitudEspecular2, 0, 0);
    ...
    m_pD3DContext->VSSetConstantBuffers(4, 1, &m_ColorDifusoC);
    m_pD3DContext->PSSetConstantBuffers(0, 1, &m_pMagnitudEspecular);
    m_pD3DContext->PSSetConstantBuffers(1, 1, &m_pMagnitudEspecular2);
