...
CDemoModel::CDemoModel(float specular)
{
    ...
    m_LightPos = m_Light;
    m_Light2 = XMFLOAT3(0.0f, 0.0f, 5.5f); // rx, ry, distance
    m_LightPos = m_Light2;
}
...
void CDemoModel::UnloadContent()
{
    ...
    m_pCameraPosCB = NULL;
    if (m_pSpecularIntensity)
        m_pSpecularIntensity->Release();
    m_pSpecularIntensity = NULL;
    if (m_pSpecularIntensity2)
        m_pSpecularIntensity2->Release();
    m_pSpecularIntensity2 = NULL;
}
...
void CDemoModel::Update()
{
    ...
    if (booleanSpecular == true)
    {
        varSpecular += 0.00005f;
        if (varSpecular > 0.4f)
        {
            booleanSpecular = false;
        }
    }
    if (booleanSpecular == false)
    {
        varSpecular -= 0.00005f;
        if (varSpecular < 0.0f)
        {
            booleanSpecular = true;
        }
    }
...
void CDemoModel::Render()
{
    ...
    m_pD3DContext->UpdateSubresource(m_pSpecularIntensity2, 0, 0, &varSpecular, 0, 0);
...
void CDemoModel::CalcSphericalCamera()
{
    ...
    XMVECTOR lightPos2 = { 0.0f,0.0f,0.0f };
