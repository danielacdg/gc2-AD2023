...
public:	
	...
	XACTINDEX cueIndex;//Indcie para apuntar a nuestro banco de sentido
	CXACT3Util m_XACT3;
    ...
    DXRR(HWND hWnd, int Ancho, int Alto)
	{
		...
		tierra = new ModeloRR(d3dDevice, d3dContext, "Assets/Planeta/planet2.obj", L"Assets/Planeta/planet.jpg", L"Assets/Planeta/planet.jpg", 0, -10);
		//
	...
	bool IniciaD3D(HWND hWnd)
	{
		...
		res = m_XACT3.LoadWaveBank(L"Win\\BancoSonidoWave.xwb");
		if (!res) {
			MessageBox(0, L"Error", L"Error al cargar el banco wav", MB_OK);
			return false;
		}
		res = m_XACT3.LoadSoundBank(L"Win\\BancoSonidoSound.xsb");
		if (!res) {
		...
		// Play XACT3 cue
		cueIndex = m_XACT3.m_pSoundBank->GetCueIndex("Sonidoguitarra");
		m_XACT3.m_pSoundBank->Play(cueIndex, 0, 0, 0);
		cueIndex = m_XACT3.m_pSoundBank->GetCueIndex("ambiente");
	...
	void Render(void)
	{
		...
		camara->posCam3P.y = terreno->Superficie(camara->posCam.x, camara->posCam.z) + 10;

		camara->UpdateCam(vel, velRightLeft, arriaba, izqder);

		if (isPointInsideSphere(camara->getPos(), tierra->getCollisionSphere(5))) {//calcular esto para que tenga sentido 
			vel = 0.0f;
		...
		model->Draw(camara->vista, camara->proyeccion, terreno->Superficie(50, 20), camara->posCam, 10.0f, 0, 'A', 1);

		tierra->Draw(camara->vista, camara->proyeccion, terreno->Superficie(0, 0) + 2.5, camara->posCam, 10.0f, 0, 'A', 1);
		//Planetas
