...
// Mapas
Texture2D alphaMapTex : register(t2);
Texture2D lightMapTex : register(t3);
Texture2D bumpMapTex : register(t4);
Texture2D specularMapTex:register(t5);
...
PS_Input VS_Main(VS_Input vertex)
{
    ...
    float c1 = cross(vsOut.norm, float3(0.0, 0.0, 1.0));
    float c2 = cross(vsOut.norm, float3(0.0, 1.0, 0.0));
...
float4 PS_Main(PS_Input frag) : SV_TARGET
{
    ...
    // AlphaMap
    float4 alphaMap = alphaMapTex.Sample(colorSampler, frag.tex0);

    // LightMap
    float4 lightMap = lightMapTex.Sample(colorSampler, frag.tex0);
        
    // BumpMap
    float4 bumpMap = bumpMapTex.Sample(colorSampler, frag.tex0);
    
    bumpMap = (bumpMap * 2.0f) - 1.0f;

    // Calculate Bump Normals
    bumpMap = (bumpMap * 2.0f) - 1.0f;
    float3 bumpNormal = (bumpMap.x + frag.tangent) 
        + (bumpMap.y + frag.binormal) 
        + (bumpMap.z + frag.norm);
    bumpNormal = normalize(bumpNormal);

    
    // SpecularMap
    float4 specularMap = specularMapTex.Sample(colorSampler, frag.tex0);
    ...
    //return textura * float(finalColor,1.0f);
