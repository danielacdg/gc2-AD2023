...
CDemoModel::CDemoModel(float specular)
{
    ...
    m_light = XMFLOAT3(0.0f, 0.0f, 0.5f);
    m_lightPos = m_light;
}
...
bool CDemoModel::LoadContent()
{
    ...
    hr = m_pD3DDevice->CreateBuffer(&constBufferDesc, 0, &m_specularIntensity);
    if (FAILED(hr)) {
        return false;
    }
...
void CDemoModel::Render()
{
    ...
    m_pD3DContext->UpdateSubresource(m_specularIntensity, 0, 0, &specularIntensity, 0, 0);
    ...
    m_pD3DContext->VSSetConstantBuffers(4, 1, &m_specularIntensity);
...
void CDemoModel::CalcSphericalCamera()
{
    ...
    m_lightPos.y = XMVectorGetX(lightPos);
    m_lightPos.z = XMVectorGetX(lightPos);
}