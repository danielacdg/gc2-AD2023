...
PS_Input VS_Main(VS_Input vertex)
{
    ...
    // Set light position
    float3 lightPos = float3(10.0f, 15.0f, -100.0f); //10.0f, 100.0f, -100.0f
    //float3 lightPos2 = float3(10.0f, -100.0f, 100.0f);
    
    float3 lightPos2 = float3(lightPosCB2[0], lightPosCB2[1]+17, lightPosCB2[2]);
...
float4 PS_Main(PS_Input frag) : SV_TARGET
{
    ...
    //Specular Color
    float specularColor = float3(0.5f,0.5f,0.5f); //color blanco comun
    float3 specularColor2 = float3(0.5f, 0.5f, 0.5f); // Color especular adicional
    
    // Diffuse color
    float3 diffuseColor = float3(0.8f, 0.0f, 0.0f);
    float3 diffuseColor2 = float3(0.0f, 0.0f, 0.8f);
