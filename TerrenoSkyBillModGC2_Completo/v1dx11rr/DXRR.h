...
public:	
	...
	bool IniciaD3D(HWND hWnd)
	{
		...
		res = m_XACT3.LoadWaveBank(L"Win\\WaveBankOF.xwb");
		if (!res) {
			MessageBox(0, L"Error", L"Error al cargar el banco wav", MB_OK);
			return false;
		}
		res = m_XACT3.LoadSoundBank(L"Win\\SoundBankOF.xsb");
		if (!res) {
		...
		// Play XACT3 cue
		cueIndex = m_XACT3.m_pSoundBank->GetCueIndex("musica");
		m_XACT3.m_pSoundBank->Play(cueIndex, 0, 0, 0);
		cueIndex = m_XACT3.m_pSoundBank->GetCueIndex("Oof");
	...
	void Render(void)
	{
		...
		if (isPointInsideSphere(camara->getPos(), tierra->getCollisionSphere(3))) {
			vel = 0.0f;
