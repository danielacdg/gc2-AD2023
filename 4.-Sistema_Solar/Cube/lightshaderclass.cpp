...
bool LightShaderClass::Initialize(ID3D11Device* device, HWND hwnd){
	...
	result = InitializeShader(device,hwnd,L"../Cube/light.vs",L"../Cube/light.ps");
	if(!result)
		return false;
...
bool LightShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
	D3DXMATRIX projectionMatrix, D3DXMATRIX traMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, traMatrix, texture, texture2,lightDirection,diffuseColor);
	if(!result)
		return false;
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
	D3DXMATRIX projectionMatrix, D3DXMATRIX traMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, D3DXVECTOR3 lightDirection,
					   D3DXVECTOR4 diffuseColor)
{
	...
	D3DXMatrixTranspose(&traMatrix, &traMatrix);
