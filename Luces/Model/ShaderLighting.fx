...
cbuffer cnChangeOccasionally : register(b3)
{
    float3 cameraPos;
...
cbuffer cnChangeOccasionally : register(b4)
{
    float3 DiffuseColor;
...
PS_Input VS_Main(VS_Input vertex)
{
    ...
    // Set light position
    float lightPos = float3(10.0f, 100.0f, -100.f);
...
float4 PS_Main(PS_Input frag) : SV_TARGET
{
    ...
    float3 DiffuseColor = float3(frag.DiffuseColor[0], frag.DiffuseColor[1], frag.DiffuseColor[2]);
