...
bool CDemoModel::LoadContent()
{
    ...
    res = CompileShader(L"ShaderLighting.fx", "VS_Main", "vs_4_0", &pVSBuffer); // usamos un .fx porque implementamos vertex y pixel dentro de este cpp y no separado como antes
    if (res == false) {
...
void CDemoModel::UnloadContent() // libera el buffer 
{
    // Cleanup
    if (m_pColorMap)
        m_pColorMap->Release();
...
void CDemoModel::Render() // dibuja la escena
{
    ...
    //m_DiffuseColor = XMFLOAT3(0.12f, 0.69f, 0.66f);
   
   
   // 1.0, .4, .4 ROJO 
    if ((R == 1.0f && G == 0.4f && B == 0.4f) || colorRojo) {
...
void CDemoModel::cambiarColor(XMFLOAT3 color) {
    ...
    if (colorRojo) {
        ...
        if (!(color.x == 0.12 && color.y == 0.69 && color.z == 0.66)) {
            color.x -= 0.01;
            color.y += 0.01;
            color.z += 0.01;
        ...
        if (!(color.x == 1.0 && color.y == .4 && color.z == .4)) {
            color.x += 0.01;
            color.y -= 0.01;
            color.z -= 0.01;
    ...
    //if (color.x <= 1.0 && color.y <= 0.4 && color.z <= 0.4) {
    ...
    //}
    //else if (color.x >= 0.12 && color.y <= 0.4 && color.z <= 0.4) {
