...
float4 PS_Main(PS_Input pix) : SV_TARGET
{
	...
	color = float4(0.2, 0.2, 0.2, 1);// ambient color float4(0.2, 0.2, 0.2, 1)

	specular = float4(0.0, 0.0, 0.0, 1.0); //specular color float4(0.0, 0.0, 0.0, 1.0)
	specularMap = specMap.Sample(colorSampler, pix.tex0);

	lightDir = -(float3(0.5f, -1.0f, 0.0f)); // lightDirection (float3(0.5f, -1.0f, 0.0f))

	lightIntensity = saturate(dot(pix.normal, lightDir));

	if (lightIntensity > 0) {
		// Determine the final diffuse color based on the diffuse color and the amount of light intensity.
		color += (float4(1.0f, 1.f, 1.f, 1.0f)/*diffuse color*/ * lightIntensity); // float4(1.0f, 1.f, 1.f, 1.0f)
