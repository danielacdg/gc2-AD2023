...
void CDemoCube::Update()
{
    ...
    if (m_LimiteHorizontal) {
        traslationx += .0006;

        if (traslationx >= 4.0) {

            m_LimiteHorizontal = false;
        }
    }
    else {
        traslationx -= .0006;
...
void CDemoCube::Render()
{
	...
	XMMATRIX t = XMMatrixTranslation(traslationx, traslationy, 0.0f);
