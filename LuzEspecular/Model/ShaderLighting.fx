...
cbuffer cbDiffuseColorRegister : register(b4)
{
    float3 diffuseColor;
};
...
cbuffer cbSpecularIntensity : register(b5)
{
    float3 SpecularIntensity;
}

cbuffer cbSpecularIntensity2 : register(b6)
{
...
struct PS_Input
{
    ...
    // ---------- Luz Especular
    float specularIntensityR : TEXTCOORD3;
};
...
float4 PS_Main(PS_Input frag) : SV_TARGET
{
    ...
    float3 diffuseColor = float3(frag.diffuseColorR[0],
        frag.diffuseColorR[1], frag.diffuseColorR[2]);

    // ---------- Color de la Luz Especular
    float3 specularColor = float3(0.5f, 0.5f, 0.5f);
    float3 specularColor2 = float3(0.5f, 0.5f, 0.5f);
