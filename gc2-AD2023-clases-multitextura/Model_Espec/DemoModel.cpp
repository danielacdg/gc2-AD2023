...
bool CDemoModel::LoadContent()
{
    ...
    hr = ::D3DX11CreateShaderResourceViewFromFile(
        m_pD3DDevice, L"Recursos/azulejo.jpg", 0, 0, &m_pColorMap, 0);
    if (FAILED(hr)) {
    ...
    // Load AlphaMap
    hr = ::D3DX11CreateShaderResourceViewFromFile(m_pD3DDevice, L"Recursos/camino.jpg", 0, 0, &m_pAlphaMap, 0);
    if (FAILED(hr)) {
        ::MessageBox(m_hWnd, L"Unable to load alphamap", L"ERROR", MB_OK);
        return false;
    }

    // Load LightMap
    hr = ::D3DX11CreateShaderResourceViewFromFile(m_pD3DDevice, L"Recursos/lightmap.jpg", 0, 0, &m_pLightMap, 0);
    if (FAILED(hr)) {
        ::MessageBox(m_hWnd, L"Unable to load LightMap", L"ERROR", MB_OK);
        return false;
    }

    // Load BumpMap
    hr = ::D3DX11CreateShaderResourceViewFromFile(m_pD3DDevice, L"Recursos/roca_bump.jpg", 0, 0, &m_pBumpMap, 0);
    if (FAILED(hr)) {
    ...
    // Load SpecularMap
    hr = ::D3DX11CreateShaderResourceViewFromFile(m_pD3DDevice, L"Recursos/azulejo_spec.jpg", 0, 0, &m_pSpecularMap, 0);
    if (FAILED(hr)) {
        ::MessageBox(m_hWnd, L"Unable to load SpecularMap", L"ERROR", MB_OK);
        return false;
    }
