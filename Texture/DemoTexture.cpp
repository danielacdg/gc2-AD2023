...
bool CDemoTexture::LoadContent()
{
    ...
    D3D11_INPUT_ELEMENT_DESC shaderInputLayout[] =
    {
        ...
        //TextCoord
    {"TEXCOORD",0,DXGI_FORMAT_R32G32B_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0}
    ...
    if (FAILED(hr))
    {
        ::MessageBox(m_hWnd, L"Error al cargar la textura 1", L"Error", MB_OK);
        return false;
    }
    ...
    if (FAILED(hr))
    {
        ::MessageBox(m_hWnd, L"Error al cargar la textura 2", L"Error", MB_OK);
        return false;
    }
    ...
    if (FAILED(hr))
    {
        ::MessageBox(m_hWnd, L"Error al cargar el sampler", L"Error", MB_OK);
        return false;
    }
