...
bool CDemoTexture::LoadContent()
{
    ...
    // Cargamos la textura
    hr = ::D3DX11CreateShaderResourceViewFromFile(m_pD3DDevice, L"Recursos/texture.jpg", 0, 0, &m_pTextura, 0);
    if (FAILED(hr)) {
        ::MessageBox(m_hWnd, L"No se pudo cargar la textura", L"ERROR", MB_OK);
        return false;
    }

    hr = ::D3DX11CreateShaderResourceViewFromFile(m_pD3DDevice, L"Recursos/water_tx.jpg", 0, 0, &m_pTextura2, 0);  //AWA
    if (FAILED(hr)) {
        ::MessageBox(m_hWnd, L"No se pudo cargar la textura", L"ERROR", MB_OK);
        return false;
    }
...
void CDemoTexture::Render()
{
    ...
    //Set Texture
    m_pD3DContext->PSSetShaderResources(0, 1, &m_pTextura);
    m_pD3DContext->PSSetShaderResources(1, 1, &m_pTextura2);
    m_pD3DContext->PSGetSamplers(0, 1, &m_pSampler);
