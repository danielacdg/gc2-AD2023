...
bool CDemoModel::LoadContent()
{
    ...
    // Create pixel shader
    hr = m_pD3DDevice->CreatePixelShader(pPSBuffer->GetBufferPointer(), pPSBuffer->GetBufferSize(), 0, &m_pPS);
...
void CDemoModel::CalcSphericalCamera()
{
    ...
    m_LightPos2.x = XMVectorGetX(-lightPos);
    m_LightPos2.y = XMVectorGetY(-lightPos);
    m_LightPos2.z = XMVectorGetZ(-lightPos);
}