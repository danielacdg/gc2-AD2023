...
bool CObjParser::LoadFile(char* szFileName)
{
    ...
    for (i = 0; i < (int)m_nVertexCount; i++) {
        ...
        if (vni < vnbufpos) {
            ...
            m_pVertex[i].norm.x = ((m_pVertex[i].pos.y - m_pVertex[i].pos.x) / (m_pVertex[i].pos.z - m_pVertex[i].pos.y)) * 100;
            m_pVertex[i].norm.y = ((m_pVertex[i].pos.y - m_pVertex[i].pos.x) / (m_pVertex[i].pos.z - m_pVertex[i].pos.y)) * 100;
            m_pVertex[i].norm.z = ((m_pVertex[i].pos.y - m_pVertex[i].pos.x) / (m_pVertex[i].pos.z - m_pVertex[i].pos.y)) * 100;
        }*/
    }

    if (vnbufpos == 0) {
        for (int j = 0; j < (int)m_nVertexCount; j++) {
            int face = j * 3;
            if (face > m_nVertexCount)
                break;

            XMFLOAT3 a = m_pVertex[face].pos;
            XMFLOAT3 b = m_pVertex[face + 1].pos;
            XMFLOAT3 c = m_pVertex[face + 2].pos;

            XMVECTOR v1 = XMLoadFloat3(&b) - XMLoadFloat3(&a);
            XMVECTOR v2 = XMLoadFloat3(&c) - XMLoadFloat3(&a);

            XMVECTOR n = XMVector2Cross(v1, v2);
            
            m_pVertex[j].norm.x = XMVectorGetIntX(n);
            m_pVertex[j].norm.y = XMVectorGetIntY(n);
            m_pVertex[j].norm.z = XMVectorGetIntZ(n);
        }
    }
