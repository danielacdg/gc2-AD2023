...
CDemoModel::CDemoModel()
{
    ...
    m_pEspecularColorCB = NULL;
    m_EspecularColor = XMFLOAT3(0.0f, 0.0f, -5.5f);
    m_EspecularColorPos = m_EspecularColor;
...
bool CDemoModel::LoadContent()
{
    ...
    constBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;//BindFlags->
    ...
    constBufferDesc.ByteWidth = sizeof(XMFLOAT4); //te envio arreglo de flotantes posicion de camara posision de 3 pero 4 por los shaders
    ...
    constBufferDesc.ByteWidth = sizeof(XMFLOAT4); //te envio arreglo de flotantes posicion de camara posision de 3 pero 4 por los shaders
    hr = m_pD3DDevice->CreateBuffer(&constBufferDesc, 0, &m_pEspecularColorCB);
    if (FAILED(hr)) {
        return false;
    }
...
void CDemoModel::UnloadContent()
{
    ...
    m_pCameraPosCB = NULL;
    if (m_pEspecularColorCB)
        m_pEspecularColorCB->Release();
    m_pEspecularColorCB = NULL;
}
...
void CDemoModel::Render()
{
    ...
    m_pD3DContext->UpdateSubresource(m_pEspecularColorCB, 0, 0, &m_SphericalCameraPos, 0, 0);
    ...
    m_pD3DContext->VSSetConstantBuffers(4, 1, &m_pEspecularColorCB);
