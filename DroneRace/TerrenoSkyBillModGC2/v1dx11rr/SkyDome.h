...
public:
	...
	bool LoadContent(WCHAR* diffuseTex)
	{
		...
		d3dResult = (*d3dDevice)->CreateBuffer(&vertexDesc, &resourceData, &opacidadCB); //creamos buffer de opacidad

		if (FAILED(d3dResult))
		{
			MessageBox(0, L"Error", L"Error al crear opacidad buffer", MB_OK);
			return false;
		}
	...
	void Render(D3DXVECTOR3 trans)
	{
		...
		//aqui se manda el float al PS
		(*d3dContext)->UpdateSubresource(opacidadCB, 0, 0, &opacidad, 0, 0);
		(*d3dContext)->VSSetConstantBuffers(1, 1, &opacidadCB);
