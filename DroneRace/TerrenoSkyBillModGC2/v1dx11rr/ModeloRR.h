...
private:
	...
	//powerUp
	ID3D11Buffer* coloresCB;
	XMFLOAT4 colores;
	bool cambio = true;
	float fred = 1.0, fgreen = 0.0, fblue = 0.2;
...
public:
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex)
	{
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &coloresCB);
	...
	//////////
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, bool tipoCamara, bool movCam,bool powerUp)
	{
		...
		if (powerUp) {
			...
			colores = XMFLOAT4(fred, fgreen, fblue, 0.5);

		}
		else {
			colores = XMFLOAT4(fred, fgreen, fblue, 0);
		}
		...
		d3dContext->UpdateSubresource(coloresCB, 0, 0, &colores, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(5, 1, &coloresCB);
