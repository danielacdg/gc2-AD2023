...
cbuffer cbChangeOnOccasionally : register(b3)
{
    float cameraPos;
};

cbuffer cDiffuseColorRegister : register(b4)
{
    float3 diffuseColor;
};
...
PS_Input VS_Main(VS_Input vertex)
{
    ...
    vsOut.diffuseColorR = diffuseColor; //color de la luz

    return vsOut;
}


float4 PS_Main(PS_Input frag) : SV_TARGET //Como se ve afectada la luz
{
    //-- Light colors
